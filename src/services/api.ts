...
export async function sendMessage(x_user_key: string, conversationId: string, content: string) {
  try {
    const response = await api.post(
      `/messages`,
      {
...
export async function getLastMessage(conversationId: string, x_user_key: string) {
  try {
    while (true) {
      ...
      if (response.data.messages[0].userId.includes('anaboli_user_')) {
        console.warn('Last message is from a user, not a bot. Waiting 2 seconds...');
  ...
  const connectSSE = async () => {
    try {
      await fetchEventSource(
        `${import.meta.env.VITE_BOTPRESS_API_URL}/conversations/${conversationId}/listen`,
        {
          headers: {
            'x-user-key': x_user_key,
